...
target_image=/home/pi/oss/tools/jtag/stm32/led_test.bin
...
while [ 1 ]; do
    DEBUGGER_INFO=$(lsusb | grep "NXP ARM mbed")
    ./probe.sh
    if [ $? -eq 0 ]; then
        ...
        while [ 1 ]; do
            ...
            if [ $? -eq 1 ]; then
                ...
                echo "wait detach... [${DEBUGGER_INFO}]"
                sleep 0.1
            fi
        done

    else
        echo "wait attach... [${DEBUGGER_INFO}]"
        sleep 0.1
